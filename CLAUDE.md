...
### 다음 작업 계획: 스마트 레이아웃 시스템 완성
1. **이미지 업로드 시 비율 계산 및 저장**
   - MediaSelectionFlowWidget에서 이미지 선택 시 비율 계산
   - AppState에 aspect ratio 정보 추가
   - 업로드와 동시에 비율 정보 저장

2. **토글 버튼을 스마트 레이아웃으로 교체**
   - Consumer 위젯으로 AppState 변경 감지
   - _updateLayout() 메서드로 레이아웃 자동 전환
   - 사용자 인터페이스 단순화

3. **동적 박스 크기 적용**
   - MediaSelectionBoxMulti에 동적 크기 지원 추가
   - 이미지 비율에 따른 박스 높이 자동 조정
   - 화면 공간 최적화