...
#### ✅ 완료된 작업
- **VoteTimerService null check 에러 수정** (2025-08-18)
  - 문제: 만료된 투표에서 controller 제거 후 null 참조 발생
  - 해결: `getRemainingTimeStream()`에 null 체크 추가
  - 파일: `lib/services/vote_timer_service.dart` (줄 121-127)
...
#### 발견된 레거시 코드
- 🔴 BaseVoteMessageStateMixin 레거시 타이머 코드 (약 120줄)
- 🔴 VoteMessageHelper 미사용 메서드 (약 55줄)
- 🔴 GlobalNotificationManager 중복 투표 로직 (약 144줄)
- 🔴 VoteStatusService 미사용 메서드 (약 60줄)

### 🎯 리팩토링 목표

1. **코드 감소**: 약 400줄 제거
2. **복잡도 감소**: High → Medium (40% 감소)
3. **중복 제거**: 투표 처리 로직 단일화
4. **성능 개선**: 불필요한 Stream 구독 제거
