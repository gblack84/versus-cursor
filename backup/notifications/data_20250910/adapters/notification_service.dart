...
class NotificationService {
  ...
  /// Firestore 스냅샷 변경 처리
  Future<void> _handleNotificationChanges(QuerySnapshot snapshot) async {
    ...
    // Repository를 통해 변환 (Repository가 이미 새 모델로 변환하는 로직을 가지고 있음)
    try {
      // 사용자 ID를 가져와서 알림 조회
      if (snapshot.docs.isNotEmpty) {
        final firstDoc = snapshot.docs.first.data() as Map<String, dynamic>;
        final userId = firstDoc['userId'] as String?;
        
        if (userId != null) {
          // Repository를 통해 알림 가져오기
      ...
      DebugHelper.error('알림 변환 실패', error: e, tag: 'NotificationService');
    }
