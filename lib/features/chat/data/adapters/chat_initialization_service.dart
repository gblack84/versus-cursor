...
class ChatInitializationService {
  ...
  Future<List<core.Message>> loadInitialMessages({
    ...
    try {
      ...
      if (cachedMessages.isNotEmpty) {
        if (kDebugMode) {
          debugPrint(
              '[Chat Init] Loaded ${cachedMessages.length} messages from cache');
        }
        ...
        if (cachedMessages.isNotEmpty) {
          ...
          if (kDebugMode) {
            debugPrint(
                '[Chat Init] Cache loaded - Last timestamp for stream cursor: $lastLoadedTimestamp');
          }
        }
      ...
      debugPrint(
          '[Chat Init] Cache read failed, falling back to Firestore: $e');
    }
    ...
    if (kDebugMode) {
      debugPrint(
          '[Chat Init] Loaded ${initialSnapshot.docs.length} initial messages');
    }

    // 병렬로 메시지 변환
    final messageFutures = initialSnapshot.docs
        .map((doc) => _convertDocToMessage(doc, chatId: chatId));
    final messages = await Future.wait(messageFutures);

    if (kDebugMode) {
      debugPrint(
          '[Chat Init] Converted ${messages.length} messages in parallel');
    }
  ...
  Future<void> updateMessageInCache(
      String chatId, MessagesModel updatedMessage) async {
    try {
      final cachedMessages = await _cacheService.getChatMessages(chatId);

      // 메시지 찾아서 업데이트
      final index = cachedMessages
          .indexWhere((m) => m.reference.id == updatedMessage.reference.id);
  ...
  Future<core.Message> _convertDocToMessage(DocumentSnapshot doc,
      {String? chatId}) async {
