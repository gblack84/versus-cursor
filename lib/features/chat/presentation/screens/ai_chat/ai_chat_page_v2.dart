...
class _AIChatPageV2State extends State<AIChatPageV2>
      ...
      if (query.docs.isNotEmpty) {
        ...
        if (lastTimestamp['timeStamp'] != null) {
          _lastLoadedTimestamp =
              (lastTimestamp['timeStamp'] as Timestamp).toDate();
          if (kDebugMode) {
            debugPrint(
                '[AI Chat] Last loaded timestamp: $_lastLoadedTimestamp');
          }
        }
  ...
  void _startIncrementalStream() {
    ...
    if (_lastLoadedDocument != null) {
      ...
      debugPrint(
          '[AI Chat Stream] Starting stream after document: ${_lastLoadedDocument!.id}');
    } else {
      debugPrint(
          '[AI Chat Stream] Starting stream without cursor - will skip initial snapshot');
    }

    // Start new subscription with metadata changes disabled
    _messagesSubscription =
        query.snapshots(includeMetadataChanges: false).listen((snapshot) async {
      if (_isBootstrapping) {
      ...
      if (addedCount > 0 || modifiedCount > 0 || removedCount > 0) {
        debugPrint(
            '[AI Chat Stream] Changes: added=$addedCount, modified=$modifiedCount, removed=$removedCount');
        debugPrint(
            '[AI Chat Stream] Processing ${snapshot.docChanges.length} changes...');
        debugPrint(
            '[AI Chat Stream] Current cache size: ${_messageCache.length}');
      }
        ...
        switch (change.type) {
          case DocumentChangeType.added:
            ...
            if (!_messageCache.containsKey(messageId)) {
              ...
              debugPrint(
                  '[AI Chat Stream] Message already in cache, skipping: $messageId');
            }
            break;
  ...
  Future<core.Message?> _convertDocumentToMessage(DocumentSnapshot doc) async {
    // Use ChatMessageService for conversion
      ...
      try {
        ...
        if (userDoc.exists) {
          ...
          final currentUser = core.User(
            id: _currentUserId,
            name: displayName.toString().isNotEmpty
