...
import '/features/chat/data/adapters/chat_initialization_service.dart';
import '/features/chat/data/adapters/chat_scroll_service.dart';
import '/features/chat/data/adapters/chat_animation_service.dart';
import '/features/profile/data/adapters/user_cache_service.dart';
import '/features/chat/data/adapters/chat_detail_migration_service.dart';
...
class _ChatDetailWidgetV2State extends State<ChatDetailWidgetV2>
  ...
  StreamSubscription<Map<String, MessageDeliveryStatus>>?
  ...
  void _startIncrementalStream() {
    ...
    if (_lastLoadedDocument != null) {
      ...
      debugPrint(
          '[Chat Detail Stream] Starting stream after document: ${_lastLoadedDocument!.id}');
    } else if (_initService.lastLoadedTimestamp != null) {
      // 캐시에서 로드한 경우 타임스탬프 기반 커서 사용
      query = query.where('timeStamp',
          isGreaterThan: Timestamp.fromDate(_initService.lastLoadedTimestamp!));
      debugPrint(
          '[Chat Detail Stream] Starting stream after timestamp: ${_initService.lastLoadedTimestamp}');
      // 타임스탬프 커서 사용 시 초기 스냅샷 스킵하지 않음 (새 메시지만 오기 때문)
      _skipInitialSnapshot = false;
    } else {
      debugPrint(
          '[Chat Detail Stream] Starting stream without cursor - will skip initial snapshot');
    }

    _messageSubscription = query
        .snapshots(includeMetadataChanges: false)
        .listen(_onIncrementalUpdate);
  }
  ...
  void _onIncrementalUpdate(QuerySnapshot snapshot) {
    ...
    debugPrint(
        '[Chat Detail Stream] Changes detected: ${snapshot.docChanges.length}');
    debugPrint(
        '[Chat Detail Stream] Added: ${snapshot.docChanges.where((c) => c.type == DocumentChangeType.added).length}');
    debugPrint(
        '[Chat Detail Stream] Modified: ${snapshot.docChanges.where((c) => c.type == DocumentChangeType.modified).length}');
    debugPrint(
        '[Chat Detail Stream] Removed: ${snapshot.docChanges.where((c) => c.type == DocumentChangeType.removed).length}');

    // Debug logging to track stream changes
    final addedCount = snapshot.docChanges
        .where((c) => c.type == DocumentChangeType.added)
        .length;
    final modifiedCount = snapshot.docChanges
        .where((c) => c.type == DocumentChangeType.modified)
        .length;
    final removedCount = snapshot.docChanges
        .where((c) => c.type == DocumentChangeType.removed)
        .length;

    if (addedCount > 0 || modifiedCount > 0 || removedCount > 0) {
      debugPrint(
          '[Chat Stream] Changes: added=$addedCount, modified=$modifiedCount, removed=$removedCount');
      debugPrint(
          '[Chat Stream] Processing ${snapshot.docChanges.length} changes...');
    }
    ...
    switch (change.type) {
      case DocumentChangeType.added:
        ...
        if (!_isAtBottom && message.authorId != _currentUser?.id) {
          ...
          if (!_animationService.fabScaleController.isAnimating &&
              _animationService.fabScaleController.value == 0) {
      ...
      case DocumentChangeType.modified:
        ...
        if (chatId != null) {
          try {
            ...
            if (kDebugMode) {
              debugPrint(
                  '[Chat Detail] Updated message in cache: ${message.id}');
            }
          } catch (e) {
      ...
      case DocumentChangeType.removed:
        ...
        if (chatId != null) {
          try {
            ...
            if (kDebugMode) {
              debugPrint(
                  '[Chat Detail] Removed message from cache: ${message.id}');
            }
          } catch (e) {
  ...
  Future<void> _loadMoreMessages() async {
    ...
    try {
      ...
      final messageFutures =
          snapshot.docs.map((doc) => _convertDocToMessage(doc));
      ...
      uniqueOlderMessages
          .addAll(currentMessages); // Add newer messages after older ones
  ...
  void _listenToMessageStatuses() {
    ...
    _messageStatusSubscription = _lifecycleService
  ...
  Future<core.User?> _resolveUser(String userId) async {
    ...
    try {
      ...
      if (userDoc.exists) {
        ...
        final coreUser =
            ChatDetailMigrationService.convertUsersModelToCore(userModel);
  ...
  void _handleSendMediaMessage(
      String mediaUrl, String mediaType, String? localPath) async {
    if (widget.chatDocument == null) return;
              ...
              child: TextField(
                ...
                onChanged: (value) {
                  setState(() {
                    _searchQuery = value;
                    _isSearching = value
                        .isNotEmpty; // Enable highlighting when text exists
                  });
                  if (value.isEmpty) {
                    setState(() {
                      _searchResults = [];
                    ...
                    children: [
                      NotificationListener<ScrollNotification>(
                        ...
                        child: Chat(
                          ...
                          timeFormat:
                              DateFormat('h:mm a'), // AM/PM 형식 (3:30 PM)
                          onMessageSend: isAiChat ? null : _handleSendPressed,
                          onAttachmentTap:
                              isAiChat ? null : _handleAttachmentPressed,
                          builders: core.Builders(
                            ...
                            emptyChatListBuilder: (context) =>
                                ChatDetailLoadingWidgets.buildEmptyChatList(
                                    isAiChat),
                          ),
                        ),
                      ),
                      ...
                      if (_isLoadingMore)
                        Positioned(
                          ...
                          left: 0,
                          right: 0,
                          child: ChatDetailLoadingWidgets
