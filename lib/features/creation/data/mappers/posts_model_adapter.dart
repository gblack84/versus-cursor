...
class PostsModelAdapter {
  ...
  static PostCore _convertToPostCore(PostsModel postsModel, String postId) {
    return PostCore(
      ...
      description:
          postsModel.description.isNotEmpty ? postsModel.description : null,
      ...
      location: postsModel.location != null
          ? GeoPoint(
              postsModel.location!.latitude, postsModel.location!.longitude)
          : null,
    );
  }

  /// Convert PostsModel to PostContent
  static PostContent _convertToPostContent(
      PostsModel postsModel, String postId) {
    return PostContent(
      postId: postId,
      ...
      targetAudience: postsModel.targetAudience.isNotEmpty
          ? postsModel.targetAudience
          : null,
      moderation:
          postsModel.moderation.isNotEmpty ? postsModel.moderation : null,
  ...
  static PostVoting _convertToPostVoting(PostsModel postsModel, String postId) {
    return PostVoting(
      ...
      voteStatus:
          postsModel.voteStatus.isNotEmpty ? postsModel.voteStatus : 'pending',
      ...
      voteCancelledReason: postsModel.voteCancelledReason.isNotEmpty
          ? postsModel.voteCancelledReason
          : null,
      ...
      displayVotesA:
          postsModel.displayVotesA > 0 ? postsModel.displayVotesA : null,
      displayVotesB:
          postsModel.displayVotesB > 0 ? postsModel.displayVotesB : null,
      ...
      expansionStatus: postsModel.expansionStatus.isNotEmpty
          ? postsModel.expansionStatus
          : 'none',
    );
  }

  /// Convert PostsModel to PostMetrics
  static PostMetrics _convertToPostMetrics(
      PostsModel postsModel, String postId) {
    return PostMetrics(
      postId: postId,
  ...
  static Map<String, dynamic> _convertFromPostVoting(PostVoting voting) {
    return {
      ...
      if (voting.voteCompletedAt != null)
        'voteCompletedAt': voting.voteCompletedAt,
      if (voting.voteCancelledAt != null)
        'voteCancelledAt': voting.voteCancelledAt,
      if (voting.voteCancelledReason != null)
        'voteCancelledReason': voting.voteCancelledReason,
      ...
      if (voting.notificationsSentAt != null)
        'notificationsSentAt': voting.notificationsSentAt,
      ...
      'displayPercentA': voting.totalVotes > 0
          ? ((voting.displayVotesAFinal / voting.totalVotes) * 100).round()
          : 0,
      'displayPercentB': voting.totalVotes > 0
          ? ((voting.displayVotesBFinal / voting.totalVotes) * 100).round()
          : 0,
  ...
  static double _calculateEngagementRate(PostsModel postsModel) {
    final interactions = postsModel.commentcount +
        postsModel.likecount +
        postsModel.sherecount +
        postsModel.savecount;
  ...
  static double _calculateQualityScore(PostsModel postsModel) {
    final likeRatio = postsModel.likecount /
        (postsModel.likecount + 1); // Avoid division by zero
