...
class RatioCalculator {
  ...
  /// 2. 비율이 비슷하면: 평균 비율 사용
  static double calculateRepresentativeRatio(
    List<double> ratios, {
    bool enableLog = true,
  }) {
    // 빈 배열 처리
    if (ratios.isEmpty) {
      if (enableLog)
        Logger.debug('[RatioCalc] 비율 배열이 비어있음 - 기본값 1.0 반환', tag: 'Layout');
      return 1.0;
    }
    ...
    if (ratios.length == 1) {
      if (enableLog)
        Logger.debug('[RatioCalc] 단일 이미지 - 비율: ${ratios.first}', tag: 'Layout');
      return ratios.first;
    }
    ...
    if (enableLog) {
      Logger.debug('[RatioCalc] 비율 분석:', tag: 'Layout');
      Logger.debug('  - 이미지 개수: ${ratios.length}', tag: 'Layout');
      Logger.debug(
        '  - 비율들: ${ratios.map((r) => r.toStringAsFixed(3)).join(', ')}',
        tag: 'Layout',
      );
      Logger.debug(
        '  - 최소 비율: ${minRatio.toStringAsFixed(3)} (가장 세로)',
        tag: 'Layout',
      );
      Logger.debug(
        '  - 최대 비율: ${maxRatio.toStringAsFixed(3)} (가장 가로)',
        tag: 'Layout',
      );
      Logger.debug('  - 차이: ${difference.toStringAsFixed(3)}', tag: 'Layout');
    }
    ...
    if (difference > 0.5) {
      if (enableLog)
        Logger.debug(
          '[RatioCalc] 극단적 차이 감지 - 최소 비율 사용: ${minRatio.toStringAsFixed(3)}',
          tag: 'Layout',
        );
      return minRatio;
    }
    ...
    if (enableLog)
      Logger.debug(
        '[RatioCalc] 비슷한 비율 - 평균 사용: ${average.toStringAsFixed(3)}',
        tag: 'Layout',
      );
