...
/// 성능 향상을 제공합니다.
class SharedPrefsNotificationDatasource
    implements ILocalNotificationDatasource {
    ...
    for (final key in cacheKeys) {
      ...
      if (cachedJson != null) {
        try {
          ...
          final exists =
              notifications.any((notif) => notif['id'] == notificationId);
  ...
  @override
  Future<Map<String, dynamic>?> getCachedNotification(
  ...
  Future<void> cacheNotification(
    ...
    // 중복 확인 및 추가
    final index =
        cachedNotifications.indexWhere((n) => n['id'] == notificationId);
    if (index >= 0) {
      // 기존 항목 업데이트
