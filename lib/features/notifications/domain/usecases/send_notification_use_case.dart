...
class SendNotificationUseCase implements UseCase<SendNotificationParams, List<String>> {
  ...
  Future<Result<List<String>>> call(SendNotificationParams params) async {
    try {
      ...
      if (params.targetUserIds.length > maxTargets) {
        return Result.failure('Cannot send to more than $maxTargets users at once');
      }
  ...
  Future<Result<List<String>>> _sendVoteNotifications(SendNotificationParams params) async {
  ...
  Notification _createUserNotification(Notification baseNotification, String userId) {
