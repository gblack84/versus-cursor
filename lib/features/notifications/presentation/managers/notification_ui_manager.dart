...
import 'i_notification_ui_delegate.dart';
import '/core_exports.dart';
import '/features/posts/presentation/utils/debug_helper.dart';

/// 알림 UI 관리자
/// 
/// Presentation 레이어에서 알림 UI 표시를 담당합니다.
/// INotificationUIDelegate 인터페이스를 구현하여 비즈니스 로직과 분리합니다.
class NotificationUIManager implements INotificationUIDelegate {
  static final NotificationUIManager _instance = NotificationUIManager._internal();
  static NotificationUIManager get instance => _instance;
  
  NotificationUIManager._internal();
  ...
  @override
  Future<BuildContext?> waitForUIContext({Duration timeout = const Duration(seconds: 10)}) async {
  ...
  Future<void> showVotingNotification({
    ...
    if (_isShowingDialog) {
      DebugHelper.warning('이미 다이얼로그 표시 중', tag: 'NotificationUIManager');
      return;
    }
  ...
  Future<void> _showVotingDialog({
        ...
        return Dialog(
          ...
          child: VotingNotificationDialog(
            ...
            onVote: (selectedOption) async {
              DebugHelper.info('투표 완료: $selectedOption', tag: 'NotificationUIManager');
    ...
    DebugHelper.info('투표 다이얼로그 닫힘', tag: 'NotificationUIManager');
  }
  ...
  VersusBoxSizeData? createSizeDataFromAspectRatios({
    ...
    try {
      ...
      DebugHelper.warning('VersusBoxSizeData 생성 실패', tag: 'NotificationUIManager');
      return null;
    }
  }
