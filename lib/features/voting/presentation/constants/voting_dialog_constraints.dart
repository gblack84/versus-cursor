...
class VotingDialogConstraints {
  ...
  static const double minTextSize =
      16.0; // 10.0 → 12.0 → 16.0 (작은 박스에서도 가독성 보장)
  static const double defaultTextSize =
      14.0; // 12.0 → 18.0 → 14.0 (이전과 유사하게 조정)
  ...
  static Size constrainBoxSize(Size proposedSize, double scaleFactor,
    ...
    // 동적 최대 높이 계산
    final maxHeight =
        screenHeight != null ? getMaxBoxHeight(screenHeight) : 500.0;
  ...
  static double getAdaptiveTextSize(double boxHeight,
      {double baseTextSize = defaultTextSize}) {
  ...
  /// 반환값: 레이아웃 변환 필요 여부
  static bool shouldConvertLayout(
      String originalLayout, double containerWidth) {
  ...
  static Size getVoteButtonSize(double boxWidth) {
    final buttonWidth =
        (boxWidth * 0.8).clamp(80.0, 120.0); // 박스 너비의 80%, 최소 80px, 최대 120px
