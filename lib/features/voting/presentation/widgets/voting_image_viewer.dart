...
class VotingImageViewer extends StatefulWidget {
  ...
  @override
  State<VotingImageViewer> createState() =>
      _VotingImageViewerState();
}
...
class _VotingImageViewerState extends State<VotingImageViewer> {
            ...
            Column(
              children: [
                ...
                if ((effectiveUrlsA.length + effectiveUrlsB.length) > 1)
                  _buildPageIndicator(
                      effectiveUrlsA.length + effectiveUrlsB.length),
  ...
  Widget _buildDualModeViewer(
      List<String> effectiveUrlsA, List<String> effectiveUrlsB) {
    return GestureDetector(
      onHorizontalDragEnd: (details) {
        // 스와이프 속도와 방향 확인
        if (details.primaryVelocity == null) return;

        print('[VotingImageViewer] 좌우 스와이프 감지:');
          ...
          Container(
            ...
            decoration: BoxDecoration(
              ...
              border: Border.all(
                color: isSingleMode
                    ? Colors.red
                    : (_currentBoxType == 'A' ? Colors.red : Colors.green),
                width: 1.5,
              ),
            ),
            child: Text(
        ...
        children: [
              ...
              if (effectiveUrlsA.isNotEmpty) ...[
                Row(
                  ...
                  children: List.generate(effectiveUrlsA.length, (index) {
                    final isActive =
                        _currentBoxType == 'A' && index == _currentIndexInBoxA;
                    return Container(
                      ...
                      decoration: BoxDecoration(
                        color: isActive
                            ? Colors.red
                            : (isSingleMode || _currentBoxType == 'A'
              ...
              if (effectiveUrlsB.isNotEmpty) ...[
                Row(
                  ...
                  children: List.generate(effectiveUrlsB.length, (index) {
                    final isActive =
                        _currentBoxType == 'B' && index == _currentIndexInBoxB;
                    return Container(
                      margin: const EdgeInsets.symmetric(horizontal: 3.0),
                      width: isActive ? 20.0 : 6.0,
                      height: 6.0,
          ...
          Row(
            ...
            children: [
              ...
              if (_getEffectiveImageUrls('A').isNotEmpty &&
                  _getEffectiveImageUrls('B').isNotEmpty) ...[
