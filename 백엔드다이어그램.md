...
sequenceDiagram
    ...
    A1->>FS: 이미지 업로드
    FS->>A1: 업로드 URL 반환
    A1->>DB: messages_record 생성<br/>(imageUrl 포함)
    DB->>A2: 실시간 리스너 트리거
    A2->>U2: 이미지 메시지 표시
    ...
    A1->>FS: 비디오 & 썸네일 업로드
    FS->>A1: 업로드 URLs 반환
    A1->>DB: messages_record 생성<br/>(videoUrl, thumbnailUrl 포함)
    DB->>A2: 실시간 리스너 트리거
    A2->>U2: 비디오 메시지 표시
```
...
flowchart TD
    ...
    K --> L[messages_record에 저장<br/>messageType: 'vote_request']
    L --> M[채팅 목록에 표시]
    L --> N[알림 오버레이 표시]
...
classDiagram
    ...
    class VoteRequestMessage {
        ...
        +String voteImageB
        +String voteStatus
        +String messageType = "vote_request"
    }
